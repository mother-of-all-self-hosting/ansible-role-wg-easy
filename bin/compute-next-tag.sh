...
#
# Tags look like `v<WireGuard Easy version>-<release>`:
#
# - if defaults/main.yml points at a WireGuard Easy version that has never been
#   released, the release counter restarts at 0 (`v15.4.0-0`)
# - otherwise the counter is incremented (`v15.4.0-1`), but only if something
...
version="$(sed -nE 's|^wg_easy_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the WireGuard Easy version from $defaults_path"
	exit 1
fi

# The version values do not carry a leading `v` (e.g. `15.4.0`), but the tags
# do (`v15.4.0-0`). Stripping any `v` before prepending one keeps this correct
